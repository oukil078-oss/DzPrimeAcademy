...
export const HierarchyChart: React.FC = () => {
  ...
  return (
    <div className="w-full rounded-3xl border border-slate-200 dark:border-gold-500/40 bg-white dark:bg-gradient-to-b dark:from-[#090F20] dark:via-[#050812] dark:to-[#020409] p-6 sm:p-10 shadow-xl text-slate-900 dark:text-white relative overflow-hidden transition-colors duration-300">
      {/* Background Glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-gold-500/10 rounded-full blur-[120px] pointer-events-none" />

      {/* Header */}
      <div className="text-center max-w-2xl mx-auto mb-10">
        <span className="px-4 py-1 rounded-full bg-gold-500/15 border border-gold-500/40 text-gold-700 dark:text-gold-300 text-xs font-semibold uppercase tracking-wider font-arabic">
          DZ PRIME ACADEMY GOVERNANCE
        </span>
        <h2 className="text-2xl sm:text-4xl font-extrabold font-arabic text-slate-900 dark:text-transparent dark:bg-gradient-to-r dark:from-gold-300 dark:via-gold-400 dark:to-gold-600 dark:bg-clip-text mt-3">
          {t('hierarchy.title')}
        </h2>
      ...
      <div className="flex flex-col items-center">
        <motion.div
          ...
          className="relative group p-6 rounded-3xl border-2 border-gold-500 bg-gradient-to-br from-amber-50 to-white dark:from-gold-500/25 dark:via-navy-900 dark:to-navy-950 shadow-md dark:shadow-gold-glow-lg flex flex-col items-center text-center max-w-sm w-full cursor-pointer transition-all"
        >
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-gold-600 via-gold-400 to-gold-200 p-0.5 shadow-gold-glow flex items-center justify-center mb-3">
            <div className="w-full h-full bg-white dark:bg-navy-950 rounded-[14px] flex items-center justify-center">
              <Crown className="w-8 h-8 text-gold-600 dark:text-gold-400" />
            </div>
          </div>
          <h3 className="text-lg sm:text-xl font-black font-arabic text-slate-900 dark:text-gold-200">
            {t('hierarchy.ceo')}
          </h3>
          <p className="text-xs text-slate-600 dark:text-gray-300 mt-1 font-arabic">
            {t('hierarchy.ceoDesc')}
          </p>
          <span className="mt-3 px-3 py-0.5 rounded-full bg-gold-500/20 text-gold-800 dark:text-gold-300 font-mono text-[10px] font-bold">
            FOUNDER & CEO
          </span>
        </motion.div>
      ...
      <div className="pt-4 border-t border-slate-200 dark:border-gold-500/30 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5">
          {[
            ...
            return (
              <div
                key={idx}
                className="p-3 rounded-2xl bg-slate-50 dark:bg-navy-900 border border-slate-200 dark:border-gold-500/20 flex flex-col items-center text-center shadow-sm"
              >
                <Icon className="w-5 h-5 text-gold-600 dark:text-gold-400 mb-1" />
                <span className="text-[11px] font-bold font-arabic text-slate-700 dark:text-gray-200">{goal.title}</span>
              </div>
            );
          })}
        </div>
        ...
        {/* Footer Ribbon */}
        <div className="mt-6 py-2.5 px-6 rounded-2xl bg-gradient-to-r from-amber-500/20 via-gold-500/30 to-amber-500/20 border border-gold-500/50 inline-block font-arabic font-extrabold text-gold-800 dark:text-gold-200 text-xs sm:text-sm tracking-wide shadow-sm">
          🇩🇿 {t('hierarchy.slogan')}
        </div>
      </div>
    </div>
